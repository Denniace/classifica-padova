...
with st.form("nuovo_locale", clear_on_submit=True):
    nome_nuovo = st.text_input("Nome del locale:")
    indirizzo_manuale = st.text_input("Indirizzo (opzionale):")
    if st.form_submit_button("Aggiungi Locale ➕"):
        if nome_nuovo and nome_nuovo not in st.session_state.locali:
            via = indirizzo_manuale if indirizzo_manuale else trova_indirizzo_auto(nome_nuovo)
